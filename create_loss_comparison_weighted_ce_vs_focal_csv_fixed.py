#!/usr/bin/env python3
"""
Create CSV file for loss_comparison_weighted_ce_vs_focal models based on WandB logs (FIXED VERSION)
"""
...
def extract_model_info_from_logs():
    ...
    log_file = "loss_comparison_weighted_ce_vs_focal_all_logs.txt"
    ...
    try:
        ...
        for section in run_sections[1:]:  # Skip first empty section
            try:
                ...
                # Extract run name from the first line (format: "1: exp1_multi_8classes_weighted_ce")
                if lines:
                    first_line = lines[0]
                    if ":" in first_line:
                ...
                # Look for test metrics with BOTH naming conventions
                # Some models use _macro suffix, others don't
                test_patterns = {
                    'test_skin_acc': r'test_skin_acc:\s*([\d.]+)',
                    'test_skin_f1_macro': r'test_skin_f1_macro:\s*([\d.]+)',
                    'test_skin_f1': r'test_skin_f1:\s*([\d.]+)',  # Without _macro
                    'test_lesion_acc': r'test_lesion_acc:\s*([\d.]+)',
                    'test_lesion_f1_macro': r'test_lesion_f1_macro:\s*([\d.]+)',
                    'test_lesion_f1': r'test_lesion_f1:\s*([\d.]+)',  # Without _macro
                    'test_bm_acc': r'test_bm_acc:\s*([\d.]+)',
                    'test_bm_f1_macro': r'test_bm_f1_macro:\s*([\d.]+)',
                    'test_bm_f1': r'test_bm_f1:\s*([\d.]+)'  # Without _macro
                }
                
                # First try to find metrics with _macro suffix
                for metric, pattern in test_patterns.items():
                    if '_macro' in metric:
                        match = re.search(pattern, section)
                        if match:
                            value = float(match.group(1))
                            if metric == 'test_skin_f1_macro':
                ...
                # Then try to find metrics without _macro suffix (for parallel models)
                for metric, pattern in test_patterns.items():
                    if '_macro' not in metric and metric != 'test_skin_acc' and metric != 'test_lesion_acc' and metric != 'test_bm_acc':
                        match = re.search(pattern, section)
                        if match:
                            value = float(match.group(1))
                            if metric == 'test_skin_f1' and test_skin_f1 == "N/A":
                                test_skin_f1 = f"{value:.5f}"
                            elif metric == 'test_lesion_f1' and test_lesion_f1 == "N/A":
                                test_lesion_f1 = f"{value:.5f}"
                            elif metric == 'test_bm_f1' and test_bm_f1 == "N/A":
                                test_bm_f1 = f"{value:.5f}"
        ...
        # Write to CSV
        output_file = "loss_comparison_weighted_ce_vs_focal_summary_fixed.csv"
