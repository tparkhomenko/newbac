#!/usr/bin/env bash
set -euo pipefail

# Configurable env
export WANDB_MODE=online
export WANDB_ENTITY="taisiyaparkhomenko-technische-universit-t-graz"
export WANDB_PROJECT="losses_with_mixup_comparison"

EPOCHS=50
BATCH_SIZE=64
LR=1e-4
WEIGHT_DECAY=1e-4
DROPOUT=0.3
HIDDEN_DIMS="512,256"
EXPERIMENT="exp1"

# Ensure output dirs
mkdir -p backend/models/multi backend/models/parallel

# Define the list of lesion loss functions we will iterate over
LOSSES=("weighted_ce" "focal" "lmf")

# MixUp suffix
BM_ENABLED=false
if [[ -f "config.yaml" ]]; then
	BM_FLAG=$(awk '/^balanced_mixup:/{f=1;next} f && /enabled:/{print $2; exit}' config.yaml | tr 'A-Z' 'a-z')
	if [[ "${BM_FLAG}" == "true" ]]; then BM_ENABLED=true; fi
fi
SUFFIX=""
if ${BM_ENABLED}; then
	echo "Balanced MixUp enabled → suffix added"
	SUFFIX="_balanced_mixup"
else
	echo "Balanced MixUp disabled → no suffix"
fi

# Run trainer for a given mode
run_one() {
	local mode="$1"
	local loss_fn="$2"
	
	local trainer=""
	local arch_suffix=""
	if [[ "$mode" == "multi" ]]; then
		trainer="training/train_multihead.py"
		arch_suffix="multihead"
	elif [[ "$mode" == "parallel" ]]; then
		trainer="training/train_parallel.py"
		arch_suffix="parallel"
	else
		echo "Unknown mode: $mode" >&2; exit 1
	fi

	local target_dir="backend/models/${mode}"
	local logs_dir="${target_dir}/logs"
	mkdir -p "$logs_dir"
	local log_file="${logs_dir}/logs_${mode}_exp1_8classes_${loss_fn}${SUFFIX}.txt"

	echo "=== Training $mode architecture for $EXPERIMENT with 8 fine-grained lesion classes (${loss_fn}) ==="
	mkdir -p "$target_dir"

	# Launch training
	python3 "$trainer" \
		--experiment "$EXPERIMENT" \
		--epochs "$EPOCHS" \
		--batch_size "$BATCH_SIZE" \
		--lr "$LR" \
		--weight_decay "$WEIGHT_DECAY" \
		--dropout "$DROPOUT" \
		--hidden_dims "$HIDDEN_DIMS" \
		--wandb_project "$WANDB_PROJECT" \
		--wandb_name "${EXPERIMENT}_${mode}_8classes_${loss_fn}${SUFFIX}" \
		2>&1 | tee "$log_file"

	# Resolve latest run dir and copy checkpoints
	local models_root="models/${EXPERIMENT}_${arch_suffix}"
	if [[ ! -d "$models_root" ]]; then
		echo "Models dir not found: $models_root" >&2; exit 1
	fi
	local latest_run
	latest_run="$(ls -1dt "${models_root}"/*/ 2>/dev/null | head -n1 || true)"
	if [[ -z "${latest_run:-}" ]]; then
		echo "No run directory under ${models_root}" >&2; exit 1
	fi

	# Copy all three MLP checkpoints with distinct filenames per loss
	for mlp in mlp1 mlp2 mlp3; do
		local target_ckpt="${target_dir}/${mlp}_${loss_fn}${SUFFIX}.pt"
		if [[ -f "${latest_run}/best.pt" ]]; then
			cp -f "${latest_run}/best.pt" "$target_ckpt"
			echo "Saved ${mlp} checkpoint to ${target_ckpt}"
		elif [[ -f "${latest_run}/final.pt" ]]; then
			cp -f "${latest_run}/final.pt" "$target_ckpt"
			echo "Saved ${mlp} checkpoint to ${target_ckpt}"
		else
			echo "No best.pt/final.pt in ${latest_run}" >&2; exit 1
		fi
	done

	echo "Logs saved to ${log_file}"
}

modes=("multi" "parallel")

# Iterate over all desired losses and modes; update config each time
for loss_fn in "${LOSSES[@]}"; do
	if [[ -f "config.yaml" ]]; then
		sed -i "s/^lesion_loss_fn:.*/lesion_loss_fn: \"${loss_fn}\"/" config.yaml
	fi
	for mode in "${modes[@]}"; do
		run_one "$mode" "$loss_fn"
	done
done

echo ""
echo "🎉 All exp1 training completed with 8 fine-grained lesion classes using ${LESION_LOSS_FN} loss!"
echo ""
echo "📁 Models saved to:"
echo "  - backend/models/multi/mlp1_<loss>${SUFFIX}.pt, mlp2_<loss>${SUFFIX}.pt, mlp3_<loss>${SUFFIX}.pt"
echo "  - backend/models/parallel/mlp1_<loss>${SUFFIX}.pt, mlp2_<loss>${SUFFIX}.pt, mlp3_<loss>${SUFFIX}.pt"
echo ""
echo "📊 Logs saved to:"
echo "  - backend/models/multi/logs/logs_multi_exp1_8classes_<loss>${SUFFIX}.txt"
echo "  - backend/models/parallel/logs/logs_parallel_exp1_8classes_<loss>${SUFFIX}.txt"
echo ""
echo "✅ Next steps:"
echo "1. Test the new models with Quick Test"
echo "2. Verify confusion matrix shows 8x8 grid for lesion classification"
echo "3. Check frontend displays all 8 lesion types correctly"
echo "4. Verify MLP1 correctly handles skin vs not-skin separately"
