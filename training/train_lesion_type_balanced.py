...
def train_model():
    ...
    # Update run name to include max samples count
    ...
    wandb.init(
        ...
        name=run_name,
        tags=training_config['wandb']['tags'] + ["balanced", "focal_loss", "cutmix", "mixup"],
        config={
            **training_config,
            'dropout': 0.5,
            'gamma': 3.0,
    ...
    # Force maximum samples per class from config regardless of what's in the input files
    # Create balanced dataset by sampling
    class_dfs = {}
    
    # Get all samples per class
    class_counts = {}
    logger.info(f"Applying max_samples_per_class={MAX_SAMPLES_PER_CLASS} limit from config.yaml")
    
    for group in ['melanocytic', 'non-melanocytic carcinoma', 'keratosis', 'fibrous', 'vascular']:
        # Get all samples for this class from the full dataset
        group_df = full_dataset.metadata[full_dataset.metadata['lesion_group'] == group]
        class_counts[group] = len(group_df)
        
        # IMPORTANT: Force downsample to MAX_SAMPLES_PER_CLASS or use all available
        # This ensures we respect the config value regardless of what's in the CSV
        if len(group_df) > MAX_SAMPLES_PER_CLASS:
            class_dfs[group] = group_df.sample(n=MAX_SAMPLES_PER_CLASS, random_state=42)
            logger.info(f"Class {group}: FORCE downsampled from {len(group_df)} to {MAX_SAMPLES_PER_CLASS} samples")
        else:
    ...
    # Check if weights are specified in config, otherwise use defaults
    default_weights = [1.5, 1.5, 1.5, 1.2, 1.2]
    config_weights = training_config.get('class_weights', default_weights)
    
    # Log the weights being used
    logger.info(f"Using class weights: {config_weights}")
    
    manual_class_weights = torch.tensor(config_weights, device=device, dtype=torch.float)
    ...
    val_class_dfs = {}
    VAL_SAMPLES_PER_CLASS = int(MAX_SAMPLES_PER_CLASS * 0.15)  # Use 15% of training samples for validation
    ...
    for group in ['melanocytic', 'non-melanocytic carcinoma', 'keratosis', 'fibrous', 'vascular']:
        ...
        # IMPORTANT: Force downsample validation set too, respecting the config-derived limit
        if len(group_df) > VAL_SAMPLES_PER_CLASS:
            val_class_dfs[group] = group_df.sample(n=VAL_SAMPLES_PER_CLASS, random_state=42)
            logger.info(f"Validation class {group}: FORCE downsampled from {len(group_df)} to {VAL_SAMPLES_PER_CLASS} samples")
        else:
