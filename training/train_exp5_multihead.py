...
def compute_split_class_counts(metadata_csv: Path, experiment_col: str = 'exp5') -> Dict[str, Dict[str, int]]:
    import pandas as pd
...
def compute_task_class_weights(dataset: ParallelUnifiedDataset, device: torch.device) -> Dict[str, Optional[torch.Tensor]]:
    counts: Dict[str, Dict[int, int]] = {
        'skin': {0: 0, 1: 0},
        'lesion': {0: 0, 1: 0, 2: 0, 3: 0, 4: 0},
        'bm': {0: 0, 1: 0},
    }
...
def train_exp5_multihead(
    ...
    # Model
    model = MultiTaskHead(input_dim=256, hidden_dims=hidden_dims, dropout=dropout)
    model = model.to(device)

    # Class weights per task
    task_class_weights = compute_task_class_weights(train_dataset, device=device)
    ...
    criteria: Dict[str, nn.Module] = {
        ...
        'lesion': nn.CrossEntropyLoss(weight=task_class_weights['lesion']) if task_class_weights['lesion'] is not None else nn.CrossEntropyLoss(),
    ...
    try:
        split_stats = compute_split_class_counts(project_root / 'data/metadata/metadata.csv', experiment_col='exp5')
    ...
    torch.save({'model_state_dict': model.state_dict(), 'config': {'input_dim': 256, 'hidden_dims': list(hidden_dims), 'dropout': dropout, 'experiment': 'exp5'}}, final_ckpt_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment', type=str, default='exp5')
    args = parser.parse_args()

    hidden_dims_tuple = tuple(int(x) for x in args.hidden_dims.split(',') if x)
    train_exp5_multihead(
